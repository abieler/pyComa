...
def calculateColumn(nRay, dTravel, pixelSize=10**-6, iFOV = 9.39*10**-6, gFactor=2*10**-7):
...
def get_v_sun(kernelMetaFile, utcStartTime):
    ...
    state, lightTime = spice.spkezr("CHURYUMOV-GERASIMENKO", et, "J2000", "NONE", "SUN")
...
def getGFactorFromDB(v_sun=8.8, gasTemp=100,species='CO'):
    
    db = sqlite3.connect('test.sqlite')
    cur = db.cursor()
    
    DBqueryLowerV = 'SELECT v_sun, gFactor from gFactors WHERE name = "%s" AND gasTemp = %i AND v_sun < %f ORDER BY v_sun DESC' %(species, gasTemp, v_sun)
    DBqueryHigherV = 'SELECT v_sun, gFactor from gFactors WHERE name = "%s" AND gasTemp = %i AND v_sun > %f ORDER BY v_sun ASC' %(species, gasTemp, v_sun)
    ...
    gFactorInterpolated = np.interp([v_sun], [v_low,v_high], [gFactorLow, gFactorHigh*2])
